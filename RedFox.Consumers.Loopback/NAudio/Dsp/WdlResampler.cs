...
using System;
// ReSharper disable InconsistentNaming (It's not our code)
...
namespace NAudio.Dsp
{
    ...
    public class WdlResampler
    {
        ...
        private void BuildLowPass(double filtpos)
        {
                ...
                m_lp_oversize != wantinterp)
            {
                ...
                if (m_filter_coeffs.Length == allocsize)
                {
                    ...
                    var dsincpos =
                        PI / m_lp_oversize * filtpos; // filtpos is outrate/inrate, i.e. 0.5 is going to half rate
        ...
        private void SincSample(float[] outBuffer,
            int outBufferIndex,
            float[] inBuffer,
            int inBufferIndex,
            double fracpos,
        ...
        private void SincSample1(float[] outBuffer,
            int outBufferIndex,
            float[] inBuffer,
            int inBufferIndex,
            double fracpos,
        ...
        private void SincSample2(float[] outptr,
            int outBufferIndex,
            float[] inBuffer,
            int inBufferIndex,
            double fracpos,
        ...
        private class WDL_Resampler_IIRFilter
        {
            ...
            public void Apply(float[] inBuffer, int inIndex, float[] outBuffer, int outIndex, int ns, int span, int w)
            {
                ...
                while (ns-- != 0)
                {
                    ...
                    inIndex += span;
                    var outx = inx * b0 + m_hist[w, 0] * b1 + m_hist[w, 1] * b2 - m_hist[w, 2] * a1 - m_hist[w, 3] * a2;
